...
- 组合总和_39：找出candidates中可以使数字和为目标数target 的 所有不同组合

  - 注意剪枝：总和超出target或索引位置index越界

- 组合总和_40：找出candidates中可以使数字和为目标数target 的 所有不同组合，**candidates中存在重复序列**

  - 首先对序列进行排序，保证相同的数都在一起，

  - 创建used数组，用于记录该位置是否被选择过，used[i - 1] == false，说明同一树层candidates[i - 1]使用过，直接跳过

  - <img src="/Users/zyh/Library/Application Support/typora-user-images/image-20230215212633198.png" alt="image-20230215212633198" style="zoom:70%;" />
